...
import { Box, Text, Group, Button, Table, Loader, Center, Modal, TextInput } from '@mantine/core'
...
export default function DomainDataTable({ domain, system }) {
  ...
  return (
    <Box style={{ background: '#fff', border: '1px solid #ebebeb', borderRadius: 6, overflow: 'hidden', marginBottom: 16 }}>
      ...
      {/* Body */}
      <Box p={16}>
        {loading && <Center py={24}><Loader color="red" size="sm" /></Center>}
        {!loading && error && <Text size="xs" c="#ef4444">{error}</Text>}
