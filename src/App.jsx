...
export default function App() {
  ...
  const { pageData, loading: pageLoading, error: pageError, slug, loadPage } = usePageDetails()
  const { items, loading: intakeLoading, error: intakeError, stats, reload } = useIntakeItems()
  const [selectedId, setSelectedId] = useState(null)

  // Derive active item from current URL path, fall back to default
  const activeItem =
    menuItems.find(n => n.path === pathname) ??
    defaultItem ??
    null
  ...
  return (
    <Box style={{
      ...
      {/* ── Always-present app shell header ── */}
      <AppHeader
        menuItems={menuItems}
      ...
      {isInbound ? (
        intakeLoading ? (
          <Center style={{ flex: 1, flexDirection: 'column', gap: 12, background: '#141414' }}>
            <Loader color="red" size="md" />
            <Text size="sm" c="#666">Loading intake items…</Text>
          </Center>
        ) : intakeError ? (
          <Center style={{ flex: 1, flexDirection: 'column', gap: 12, background: '#141414' }}>
            <Text size="sm" c="#ef4444" fw={600}>Failed to load intake items</Text>
            <Text size="xs" c="#666">{intakeError}</Text>
            <Button size="sm" onClick={reload} leftSection={<IconRefresh size={14} />}
          ...
          <>
            ...
            <Box style={{ display: 'flex', flex: 1, overflow: 'hidden', minHeight: 0 }}>
              <LeftRail items={items} stats={stats} selectedId={selectedItem?.id} onSelect={setSelectedId} />
